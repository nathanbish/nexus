...
export default function Home() {
  ...
  const [tasks, setTasks] = useState([
    { id: 1, text: "Review Q3 board deck", category: "Work", priority: "high", done: false },
    { id: 2, text: "Respond to contract renewal email", category: "Legal", priority: "high", done: false },
    { id: 3, text: "Schedule strategy sync meeting", category: "Admin", priority: "medium", done: false },
    { id: 4, text: "Review monthly budget", category: "Finance", priority: "low", done: false },
  ]);
  ...
  const scanDeadlines = async () => {
    if (!emails.length) { alert("Load your inbox first."); return; }
    setEmailLoading(true);
    try {
        ...
        {tab === "Overview" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
              <Card><Label>UNREAD EMAILS</Label><Big>{emails.length || "—"}</Big><Sub>{emails.length ? `${emails.length} loaded` : "Load inbox to sync"}</Sub></Card>
        ...
        {tab === "Email" && (
          <div>
            ...
            {selectedEmail ? (
              ...
              <div>
                {emails.length === 0 && <div style={{ color: "#888", textAlign: "center", marginTop: 40 }}>No emails loaded yet</div>}
                {emails.map(e => (
