...
export async function chat(messages, systemPrompt, maxTokens = 4096) {
...
export async function draftReply(email, userInstruction) {
  return chat([{
    role: 'user',
    content: `Draft a professional email reply. Write in plain text only — no markdown, no asterisks, no bold, no subject line. Just the email body starting with the greeting.\n\nOriginal email:\nFrom: ${email.from}\nSubject: ${email.subject}\n${email.body}\n\nMy instructions: ${userInstruction || 'Write a professional, concise reply'}`,
  }], SYSTEM_EMAIL)
}
...
3. "junk": emails that are safe to delete — newsletters, marketing, automated notifications, receipts, order confirmations, digest emails, promotional offers, social media notifications, or anything Nathan clearly does not need to keep. Each object: { index, from, subject }.
...
${list}`,
  ...
  try {
    ...
    console.error('scanEmails parse error:', e.message)
    return { items: [], needsReply: [], junk: [] }
  }
}
